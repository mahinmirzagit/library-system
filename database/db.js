...
if (isPostgres) {
  ...
  // Also converts basic SQLite specific syntax if needed
  function translateQuery(query) {
    ...
    if (translated.trim().toUpperCase().startsWith("INSERT") && !translated.toUpperCase().includes("RETURNING")) {
      // Very basic detection, might need refinement for complex inserts
      translated += " RETURNING id";
    }
    
    // Convert SQLite DATETIME functions and generic keywords
  ...
  db = {
    ...
    run: function(query, params, cb) {
      ...
      pool.query(sql, params)
        .then(res => {
          // Emulate sqlite3 'this' context for callbacks
          const context = {
    ...
    exec: async (query, cb) => {
      try {
        // Postgres can handle multiple statements if separated by semicolons
...
function initializePostgres() {
  ...
  // Check for 'books' table in Postgres
  db.get(
